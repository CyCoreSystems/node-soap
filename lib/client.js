...
Client.prototype._invoke = function(method, arguments, location, callback, options) {
    ...
    // Defining SOAPAction gives Andrew's GWaaS system fits,
    // so we are manually disabling it, since we have no other
    // way of reliably identifying when to send it.
    // The specification allows it to be blank, in any case.
    if (self.wsdl.definitions['$name'] == 'TwoWayVoiceCallbacksService') {
        headers.SOAPAction = '""';
    }
